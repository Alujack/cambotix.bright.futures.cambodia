...
export default function ProjectsPage() {
  ...
  return (
    <main>
      <PageIntro
        eyebrow={copy["projects"]["001 eyebrow: Our Projects"]}
        title={copy["projects"]["002 title: Practical support that creates opportunity."]}
        description={copy["projects"]["003 description: Explore four community-led projects supportin"]}
      />
            ...
            <article id={project.slug} key={project.slug} className="scroll-mt-36 overflow-hidden rounded-[30px] border border-orange-100 bg-white shadow-[0_18px_55px_rgb(91_60_37/6%)]">
              <div className="grid lg:grid-cols-2">
                <div className={`relative min-h-[320px] sm:min-h-[440px] ${index % 2 === 1 ? 'lg:order-2' : ''}`}>
                  ...
                  <span className="absolute left-5 top-5 rounded-full bg-white/90 px-4 py-2 text-xs font-extrabold text-[#d95121] shadow-sm backdrop-blur">{copy["projects"]["004 Project"]} {project.number}</span>
                </div>
                <div className="flex flex-col justify-center p-6 sm:p-10 lg:p-12">
                  ...
                  <Link href={`/projects/${project.slug}`} className="mt-7 inline-flex w-fit rounded-2xl bg-[#201a15] px-6 py-3.5 font-bold text-white transition hover:-translate-y-0.5 hover:bg-[#342b24]">{copy["projects"]["005 Explore this project"]}</Link>
                </div>
              </div>
            </article>
          ))}
      ...
      <section className="bg-[#201a15] text-white">
        <div className="mx-auto flex max-w-7xl flex-col items-start justify-between gap-8 px-4 py-14 sm:px-6 lg:flex-row lg:items-center lg:px-8">
          <div className="max-w-3xl">
            <p className="text-sm font-extrabold uppercase tracking-[0.16em] text-[#ff8b62]">{copy["projects"]["006 Support the work"]}</p>
            <h2 className="mt-3 text-3xl font-extrabold tracking-tight sm:text-4xl">{copy["projects"]["007 Help one of these projects continue."]}</h2>
            <p className="mt-3 leading-relaxed text-stone-300">{copy["projects"]["008 Volunteer, donate supplies, fundraise, or make a f"]}</p>
          </div>
          <Link href={copy["projects"]["009 href: /contact"]} className="w-full rounded-2xl bg-[#f26b3a] px-7 py-4 text-center font-extrabold text-white transition hover:bg-[#df5524] sm:w-auto">{copy["projects"]["010 Contact our team"]}</Link>
        </div>
      </section>
    </main>
  );
